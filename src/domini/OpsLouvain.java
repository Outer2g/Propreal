...
public class OpsLouvain {
	...
	public static Vector<Pair<Integer, Float>> neighbors(Graph<Node, Edge> g, int i){
	...
	public static float nb_selfloops(Graph<Node, Edge> g, Node n){
		if(g.getAdjacentNodesTo(n).contains(n)) return g.getEdge(n, n).getWeight();
		else return 0;
	}
	
	public static float weighted_degree(Graph<Node, Edge> g, Node n){
	...
	public static float getTotalWeight(Graph<Node, Edge> g){
